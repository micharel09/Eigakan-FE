...
import { Input, Select, Button, Upload, Card, notification, Progress } from "antd"
import { UploadOutlined, PlusOutlined, DeleteOutlined, SaveOutlined, ClockCircleOutlined } from "@ant-design/icons"
    ...
    <div className="p-6 mx-auto bg-white rounded-lg shadow-lg max-w-4xl">
      ...
      <div className="space-y-6">
        {medias.map((media, index) => (
          <Card key={index} className="overflow-hidden shadow-sm" bodyStyle={{ padding: 0 }}>
            <div className="p-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium">Media Item #{index + 1}</h3>
                <Button danger onClick={() => removeMedia(index)} icon={<DeleteOutlined />}>
                  Remove
                </Button>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex items-center gap-4">
                  <Upload
                    ...
                    disabled={uploading}
                  >
                    <Button icon={<UploadOutlined />} disabled={uploading && currentUploadIndex === index}>
                      {uploading && currentUploadIndex === index ? "Uploading..." : "Select File"}
                    </Button>
                  </Upload>
                  ...
                  <div className="bg-gray-100 rounded-md p-2 overflow-hidden">
                    {media.type === "FILMVIP" || media.type === "TRAILER" ? (
                      <div className="aspect-video w-full max-w-md mx-auto">
                        <iframe src={media.url} className="w-full h-full rounded-md" allowFullScreen></iframe>
                      </div>
                    ) : (
                      <img
        ...
        <Button type="dashed" onClick={addMedia} block icon={<PlusOutlined />} disabled={uploading} className="h-16">
          Add New Media
        </Button>
