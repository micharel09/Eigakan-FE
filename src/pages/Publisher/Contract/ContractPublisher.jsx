...
import { SearchOutlined, PlusOutlined, CheckCircleOutlined, ClockCircleOutlined, CloseCircleOutlined, FolderOutlined, CalendarOutlined, FieldTimeOutlined, SyncOutlined } from "@ant-design/icons";
...
const ContractPublisher = () => {
  ...
  // Kiểm tra & cập nhật trang hợp lệ
  useEffect(() => {
    const totalPages = Math.ceil(totalContracts / pageSize);
    
    // Nếu số trang thay đổi & currentPage không hợp lệ, reset về trang 1
    if (currentPage > totalPages && totalPages > 0) {
      setCurrentPage(1);
    } else {
      fetchContract(currentPage, pageSize);
    }
  }, [currentPage, totalContracts]); // Lắng nghe cả totalContracts
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
          {contracts.map((contract) => (
            <Link key={contract.id} to={`/publisher/contract/${contract.id}`} className="flex justify-center">
              <Card
                hoverable
                cover={
                  <div className="relative pt-[100%]">
                    <img
                      ...
                      src={contract.movie?.medias?.length > 0 ? contract.movie.medias[0].url : "/placeholder.svg"}
                      className="absolute top-0 left-0 w-full h-full object-cover rounded-t-lg"
                    />
                    <div className="absolute top-2 right-2">
                      {contract.status === "SIGNED" ? (
                        <Tag icon={<CheckCircleOutlined />} color="green">Signed</Tag>
              ...
              >
                <Card.Meta
                  title={<span className="font-semibold">Contract of {contract.movie?.title || "Unknown Title"}</span>}
                  description={
                    <div className="text-gray-500 space-y-2">
                      <div className="flex justify-between">
                        <p><CalendarOutlined /> <strong>Distributor:</strong> {contract.distributorName || "Unknown"}</p>
                        <p><CalendarOutlined /> <strong>Email:</strong> {contract.user?.email || "N/A"}</p>
                      </div>
                      <div className="flex justify-between">
                        <p><FieldTimeOutlined /> <strong>Start:</strong> {new Date(contract.startDate).toLocaleDateString()}</p>
                        <p><FieldTimeOutlined /> <strong>End:</strong> {new Date(contract.endDate).toLocaleDateString()}</p>
                      </div>
                      <p><ClockCircleOutlined /> <strong>Duration:</strong> {contract.duration} days</p>
                    </div>
                  }
                />
              </Card>
